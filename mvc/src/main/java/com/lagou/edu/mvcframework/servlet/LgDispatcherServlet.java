...
//        判断请求中是否包含username属性
        ...
        if(username == null){
            resp.getWriter().write("请登录(请求后面带上username属性)");
            return;
        }

//        根据handle判断请求中的用户有没有访问权限
        if(!judgePermission(handler,username)){
            resp.getWriter().write("当前用户没有访问权限");
            return;
        }
    ...
    private boolean judgePermission(Handler handler, String username) {
        List names = new ArrayList();
        Class<?> aClass = handler.getController().getClass();
//        获取controller上LagouSecurity的value
